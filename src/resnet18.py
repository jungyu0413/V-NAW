...
models_dir = os.path.expanduser('Static_model/backbone')
model_name = {
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x)  # Fit official version (PyTorch)
        x = torch.flatten(x, 1)
        x = self.fc(x)
...
def resnet50(pretrained=True, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], num_classes=8631, **kwargs)
    # if pretrained:
